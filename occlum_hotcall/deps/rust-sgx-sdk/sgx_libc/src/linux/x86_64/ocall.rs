...
const HOTCALL_INITIALIZER: HotCall = HotCall {
    responderThread: 0 as *mut c_void,
...
//pub static mut hotOcall: HotCall = HOTCALL_INITIALIZER;
pub static mut p_hotOcall: u64 = 0;

const MAX_OCALL_ALLOC_SIZE: size_t = 0x4000; //16K
//const MAX_OCALL_ALLOC_SIZE: size_t = 0x10000; //64K
...
extern "C" {
    ...
    pub fn setsockopt_hotcall(
        data: *mut setsockoptParams,
    ) -> sgx_status_t;

    
    pub fn accept4_hotcall(
        data: *mut accept4Params,
    ) -> sgx_status_t;

    pub fn getsockopt_hotcall(
        data: *mut getsockoptParams,
    ) -> sgx_status_t;

    pub fn shutdown_hotcall(
        data: *mut shutdownParams,
    ) -> sgx_status_t;

    pub fn fcntl_arg1_hotcall(
        data: *mut fcntl_arg1Params,
    ) -> sgx_status_t;
    ...
    pub fn close_hotcall(
        data: *mut closeParams,
    ) -> sgx_status_t;

    pub fn epoll_wait_hotcall(
        data: *mut epoll_waitParams,
    ) -> sgx_status_t;
...
pub unsafe fn read(fd: c_int, buf: *mut c_void, count: size_t) -> ssize_t {
    ...
    HotCall_requestCall( hotOcall_p, 4, data_p );
    let status = sgx_status_t::SGX_SUCCESS;*/

    //let data_p = &mut data as *mut accept4Params;
    //let status = accept4_hotcall(data_p);
...
pub unsafe fn fcntl_arg1(fd: c_int, cmd: c_int, arg: c_int) -> c_int {
    ...
    HotCall_requestCall( hotOcall_p, 5, data_p );
    let status = sgx_status_t::SGX_SUCCESS;

   // let data_p = &mut data as *mut fcntl_arg1Params;
   // let status = fcntl_arg1_hotcall(data_p);
    // hot-calls end
...
pub unsafe fn close(fd: c_int) -> c_int {
    ...
    HotCall_requestCall( hotOcall_p, 11, data_p );
    let status = sgx_status_t::SGX_SUCCESS;
...
pub unsafe fn accept4(
    ...
    // hot-calls begin
    let mut data = accept4Params {
        result_p: &mut result,
        error_p: &mut error,
        sockfd: sockfd,
        addr: addr,
        addrlen_in: len_in,
        addrlen_out: &mut len_out,
        flags: flags,
    };
    ...
    HotCall_requestCall( hotOcall_p, 3, data_p );
    let status = sgx_status_t::SGX_SUCCESS;
...
pub unsafe fn setsockopt(
    ...
    HotCall_requestCall( hotOcall_p, 0, data_p );
    let status = sgx_status_t::SGX_SUCCESS;

   // let data_p = &mut data as *mut setsockoptParams;
  //  let status = setsockopt_hotcall(data_p);
    // hot-calls end
...
pub unsafe fn getsockopt(
    ...
    HotCall_requestCall( hotOcall_p, 1, data_p );
    let status = sgx_status_t::SGX_SUCCESS;


   // let data_p = &mut data as *mut getsockoptParams;
   // let status = getsockopt_hotcall(data_p);
    // hot-calls end
...
pub unsafe fn shutdown(sockfd: c_int, how: c_int) -> c_int {
    ...
    // hot-calls begin
    /*let mut data = shutdownParams {
        result_p: &mut result as *mut c_int,
        error_p: &mut error as *mut c_int,
        sockfd: sockfd,
        how: how,
    };
    ...
    HotCall_requestCall( hotOcall_p, 2, data_p );
    let status = sgx_status_t::SGX_SUCCESS;*/

    //let data_p = &mut data as *mut shutdownParams;
    //let status = shutdown_hotcall(data_p);
    // hot-calls end
...
pub unsafe fn epoll_wait(
    ...
    HotCall_requestCall( hotOcall_p, 13, data_p );
    let status = sgx_status_t::SGX_SUCCESS;*/
    // hot-calls end
