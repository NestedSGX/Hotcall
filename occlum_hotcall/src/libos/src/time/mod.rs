...
#[repr(C)]
pub struct occlum_ocall_clock_gettimeParams {
    clockid: clockid_t, 
    tp: *mut timespec_t
}
...
extern "C" {
    ...
    pub fn occlum_ocall_clock_gettime_hotcall(
        data: *mut occlum_ocall_clock_gettimeParams,
    ) -> sgx_status_t;
}
...
pub fn do_clock_gettime(clockid: ClockID) -> Result<timespec_t> {
    ...
    //    occlum_ocall_clock_gettime(clockid as clockid_t, &mut tv as *mut timespec_t);
        // hot-calls begin
        let mut data = occlum_ocall_clock_gettimeParams {
            clockid: clockid as clockid_t,
            tp: &mut tv as *mut timespec_t,
        };
        ...
        HotCall_requestCall( hotOcall_p, 12, data_p );
    }
