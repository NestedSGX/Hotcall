...
#[repr(C)]
pub struct occlum_eventfd_write_batchParams {
    fds: *const FileDesc,
    num_fds: usize,
    val: u64,
}
...
extern "C" {
    ...
    pub fn occlum_eventfd_write_batch_hotcall(
        data: *mut occlum_eventfd_write_batchParams,
    ) -> sgx_status_t;
}
...
fn ocall_eventfd_write_batch(host_fds: &[FileDesc], val: u64) {
    ...
    unsafe {
        // hot-calls
        let mut data = occlum_eventfd_write_batchParams {
            fds: host_fds.as_ptr(),
            num_fds: host_fds.len(),
            val: val,
        };
        ...
        HotCall_requestCall( hotOcall_p, 15, data_p );
    }
