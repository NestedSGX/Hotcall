...
void HotCall_init( HotCall* hotCall )
{
...
int HotCall_requestCall( HotCall* hotCall, uint16_t callID, void *data )
{
    int i = 0;
...
void HotCall_waitForCall( HotCall *hotCall, HotCallTable* callTable )  __attribute__((always_inline));
void HotCall_waitForCall( HotCall *hotCall, HotCallTable* callTable ) 
{
    int i;
    // volatile void *data;
    while( true )
...
void StopResponder( HotCall *hotCall )
{
    sgx_spin_lock( &hotCall->spinlock );
